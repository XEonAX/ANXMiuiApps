    ...
    void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (event.getKeyData(possibleChars) || keyCode == 67) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (((SupportMenu.SUPPORTED_MODIFIERS_MASK & modifierState) == (SupportMenu.SUPPORTED_MODIFIERS_MASK & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != 0 && ((shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2] || (qwerty && shortcutChar == 8 && keyCode == 67)) && item.isEnabled())) {
                    items.add(item);
                }
            }
        }
