...
public final class OkHostnameVerifier implements HostnameVerifier {
    ...
    public boolean verifyHostname(String hostname, String pattern) {
        if (hostname == null || hostname.length() == 0 || hostname.startsWith(SymbolExpUtil.SYMBOL_DOT) || hostname.endsWith("..") || pattern == null || pattern.length() == 0 || pattern.startsWith(SymbolExpUtil.SYMBOL_DOT) || pattern.endsWith("..")) {
            return false;
        }
        if (!hostname.endsWith(SymbolExpUtil.SYMBOL_DOT)) {
            hostname = hostname + '.';
        }
        if (!pattern.endsWith(SymbolExpUtil.SYMBOL_DOT)) {
            pattern = pattern + '.';
        }
