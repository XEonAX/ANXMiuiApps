...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (networkRequest == null && cacheResponse == null) {
            return new Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(SecExceptionCode.SEC_ERROR_DYN_STORE_GET_DATA_FILE_KEY_FAILED).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        return response.newBuilder().body(new RealResponseBody(response.header(Constants.Protocol.CONTENT_TYPE), response.body().contentLength(), Okio.buffer(cacheWritingSource))).build();
    }
    ...
    static boolean isContentSpecificHeader(String fieldName) {
        return Constants.Protocol.CONTENT_LENGTH.equalsIgnoreCase(fieldName) || Constants.Protocol.CONTENT_ENCODING.equalsIgnoreCase(fieldName) || Constants.Protocol.CONTENT_TYPE.equalsIgnoreCase(fieldName);
    }
}
