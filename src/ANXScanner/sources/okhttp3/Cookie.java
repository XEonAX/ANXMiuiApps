...
public final class Cookie {
        ...
        while (pos < limit) {
            ...
            if (attributeName.equalsIgnoreCase("expires")) {
                ...
                if (attributeName.equalsIgnoreCase("max-age")) {
                    ...
                    if (attributeName.equalsIgnoreCase(LoginConstants.DOMAIN)) {
                        try {
    ...
    private static long parseExpires(String s, int pos, int limit) {
        ...
        if (year >= 0 && year <= 69) {
            year += SecExceptionCode.SEC_ERROR_PAGETRACK;
        }
        if (year < SecExceptionCode.SEC_ERROR_SAFETOKEN_INVALID_PARAM) {
    ...
    private static int dateCharacterOffset(String input, int pos, int limit, boolean invert) {
        for (int i = pos; i < limit; i++) {
            ...
            if ((c >= 32 || c == 9) && c < DecodeType.ONE && ((c < 48 || c > 57) && ((c < 97 || c > SecExceptionCode.SEC_ERROR_INIT_NO_DATA_FILE) && ((c < 65 || c > 90) && c != 58)))) {
                dateCharacter = false;
            } else {
                dateCharacter = true;
            }
    ...
    private static String parseDomain(String s) {
        if (s.endsWith(SymbolExpUtil.SYMBOL_DOT)) {
            throw new IllegalArgumentException();
        }
        if (s.startsWith(SymbolExpUtil.SYMBOL_DOT)) {
            s = s.substring(1);
        }
    ...
    String toString(boolean forObsoleteRfc2965) {
        ...
        if (!this.hostOnly) {
            ...
            if (forObsoleteRfc2965) {
                result.append(SymbolExpUtil.SYMBOL_DOT);
            }
