...
class ProviderAwakeModule implements IAwakeModule {
    ...
    public void doAwake(Context context, AwakeInfo packageInfo) {
        if (packageInfo != null) {
            ...
            AwakeUploadHelper.uploadData(context, "provider", ErrorCode.MAX_DURATION_REACHED, "A receive incorrect message");
        }
    }
    ...
    private void awakeByProvider(Context context, String authority, String awakeInfo) {
        if (context == null || TextUtils.isEmpty(authority) || TextUtils.isEmpty(awakeInfo)) {
            if (TextUtils.isEmpty(awakeInfo)) {
                AwakeUploadHelper.uploadData(context, "provider", ErrorCode.MAX_DURATION_REACHED, "argument error");
            } else {
            ...
            try {
                if (TextUtils.isEmpty(info)) {
                    AwakeUploadHelper.uploadData(context, awakeInfo, ErrorCode.MAX_DURATION_REACHED, "info is empty");
                    return;
                }
                ...
                if (TextUtils.isEmpty(result) || !"success".equals(result)) {
                    AwakeUploadHelper.uploadData(context, awakeInfo, ErrorCode.MAX_DURATION_REACHED, "A is fail to help B's provider");
                    return;
                }
                ...
                MyLog.e(e);
                AwakeUploadHelper.uploadData(context, awakeInfo, ErrorCode.MAX_DURATION_REACHED, "A meet a exception when help B's provider");
            }
        } else {
    ...
    private void parseProvider(Context context, String uri) {
        try {
            if (TextUtils.isEmpty(uri) || context == null) {
                AwakeUploadHelper.uploadData(context, "provider", ErrorCode.MAX_DURATION_REACHED, "B get a incorrect message");
                return;
            }
            ...
            if (TextUtils.isEmpty(awakeInfo)) {
                ...
                AwakeUploadHelper.uploadData(context, awakeInfo, ErrorCode.MAX_SIZE_REACHED, "play with provider successfully");
            }
        } catch (Exception e) {
            AwakeUploadHelper.uploadData(context, "provider", ErrorCode.MAX_DURATION_REACHED, "B meet a exception" + e.getMessage());
        }
    }
}
