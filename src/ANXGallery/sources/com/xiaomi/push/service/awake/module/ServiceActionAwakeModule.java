...
class ServiceActionAwakeModule implements IAwakeModule {
    ...
    public void doAwake(Context context, AwakeInfo packageInfo) {
        if (packageInfo != null) {
            ...
            AwakeUploadHelper.uploadData(context, "service", ErrorCode.MAX_DURATION_REACHED, "A receive incorrect message");
        }
    }
    ...
    private void parseService(Service service, Intent intent) {
        ...
        if (TextUtils.isEmpty(awakeInfo)) {
            ...
            AwakeUploadHelper.uploadData(service.getApplicationContext(), awakeInfo, ErrorCode.MAX_SIZE_REACHED, "play with service successfully");
        }
    }
}
