...
/* renamed from: miui.util.FeatureParser */
public class C0016FeatureParser {
    ...
    private static void read() {
        ...
        try {
            if (!"cancro".equals(C0015Build.DEVICE)) {
                StringBuilder stringBuilder2 = new StringBuilder();
                stringBuilder2.append(C0015Build.DEVICE);
                stringBuilder2.append(ThemeZipFile.THEME_VALUE_FILE_SUFFIX);
                fileName = stringBuilder2.toString();
            } else if (C0015Build.MODEL.startsWith("MI 3")) {
                fileName = "cancro_MI3.xml";
            } else if (C0015Build.MODEL.startsWith("MI 4")) {
                fileName = "cancro_MI4.xml";
            }
            AssetManager assets = Resources.getSystem().getAssets();
            stringBuilder = new StringBuilder();
            stringBuilder.append("device_features/");
            ...
            stringBuilder3.append("device_features/");
            stringBuilder3.append(",it may be in ");
            stringBuilder3.append("/system/etc/device_features");
            Log.i("FeatureParser", stringBuilder3.toString());
        } catch (XmlPullParserException e2) {
        } catch (Throwable th) {
        }
        if (inputStream == null) {
            File file = new File("/system/etc/device_features", fileName);
            if (file.exists()) {
                ...
                stringBuilder.append(fileName);
                Log.e("FeatureParser", stringBuilder.toString());
                if (inputStream != null) {
                    try {
                        inputStream.close();
        ...
        for (int type = parser.getEventType(); 1 != type; type = parser.next()) {
            switch (type) {
                ...
                case 3:
                    ...
                    if (!"integer-array".equals(end_tag_name)) {
                        if ("string-array".equals(end_tag_name)) {
